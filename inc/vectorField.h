...
{
	public:
	VectorField(const sf::Vector2f& dim,
		   const sf::Vector2u& resolution);
	virtual ~VectorField();

	const VectorField& operator+=(const VectorField& other);

	void setMaxVectorLength(float length);
	float getMaxVectorLength() const;
	void setMinVectorLength(float length);
	float getMinVectorLength() const;
